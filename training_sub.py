...
class EarlyStopping_loss:
    ...
    def __call__(self, loss_val, model, epoch, optimizer, loss_train):
        score = -loss_val

        if self.best_score is None:
            self.best_score = score
            self.save_checkpoint(loss_val, model, epoch, optimizer, loss_train)
        elif score < self.best_score + self.delta:
            self.counter += 1
            self.trace_func(f"EarlyStopping counter: {self.counter} out of {self.patience}")
            self.flog.write(f"EarlyStopping counter: {self.counter} out of {self.patience}\n")
    ...
    def save_checkpoint(self, loss_val, model, epoch, optimizer, loss_train):
...
def write_train_log(f_log, epoch, each_loss_train, each_loss_val, epoch_start_time):
    """epochごとのlogを出力する
    Args:
